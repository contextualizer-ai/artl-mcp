...
from artl_mcp.main import cli, create_mcp, main
...
class TestCLIFunction:
    ...
    def test_cli_with_pmid_search_custom_max_results(self, mock_search):
        ...
        runner = CliRunner()
        result = runner.invoke(cli, ['--pmid-search', 'custom query', '--max-results', '5'])
    ...
    def test_cli_with_both_options_error(self):
        ...
        result = runner.invoke(cli, [
            '--doi-query', '10.1038/test',
            '--pmid-search', 'test query'
        ])
        
        assert result.exit_code != 0
        assert "Cannot use both --doi-query and --pmid-search simultaneously" in result.output
