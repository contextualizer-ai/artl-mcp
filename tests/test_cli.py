...
class TestDOICommands:
    ...
    def test_get_doi_metadata_cmd_success(self, mock_get_doi_metadata):
        ...
        mock_get_doi_metadata.return_value = {"DOI": "10.1038/nature12373", "title": "Test Article"}
    ...
    def test_get_doi_fetcher_metadata_cmd_success(self, mock_get_doi_fetcher_metadata):
        ...
        runner = CliRunner()
        result = runner.invoke(get_doi_fetcher_metadata_cmd, ['--doi', '10.1038/nature12373', '--email', 'test@example.com'])
        
        assert result.exit_code == 0
        mock_get_doi_fetcher_metadata.assert_called_once_with('10.1038/nature12373', 'test@example.com')
...
class TestUnpaywallCommands:
    ...
    def test_get_unpaywall_info_cmd_success_strict(self, mock_get_unpaywall_info):
        ...
        mock_get_unpaywall_info.return_value = {"is_oa": True, "genre": "journal-article"}
        
        runner = CliRunner()
        result = runner.invoke(get_unpaywall_info_cmd, [
        ...
        assert result.exit_code == 0
        mock_get_unpaywall_info.assert_called_once_with('10.1038/nature12373', 'test@example.com', True)
        output = json.loads(result.output.strip())
...
class TestFullTextCommands:
    ...
    def test_get_full_text_from_doi_cmd_success(self, mock_get_full_text_from_doi):
        ...
        runner = CliRunner()
        result = runner.invoke(get_full_text_from_doi_cmd, [
            '--doi', '10.1038/nature12373',
            '--email', 'test@example.com'
        ])
        
        assert result.exit_code == 0
        mock_get_full_text_from_doi.assert_called_once_with('10.1038/nature12373', 'test@example.com')
    ...
    def test_get_full_text_info_cmd_success(self, mock_get_full_text_info):
        ...
        mock_get_full_text_info.return_value = {"success": True, "info": "Found full text"}
        
        runner = CliRunner()
        result = runner.invoke(get_full_text_info_cmd, [
            '--doi', '10.1038/nature12373',
            '--email', 'test@example.com'
        ])
        
        assert result.exit_code == 0
        mock_get_full_text_info.assert_called_once_with('10.1038/nature12373', 'test@example.com')
...
class TestPDFCommands:
    ...
    def test_get_text_from_pdf_url_cmd_success(self, mock_get_text_from_pdf_url):
        ...
        runner = CliRunner()
        result = runner.invoke(get_text_from_pdf_url_cmd, [
            '--pdf-url', 'https://example.com/paper.pdf',
            '--email', 'test@example.com'
        ])
        
        assert result.exit_code == 0
        mock_get_text_from_pdf_url.assert_called_once_with('https://example.com/paper.pdf', 'test@example.com')
    ...
    def test_extract_pdf_text_cmd_success(self, mock_extract_pdf_text):
        ...
        runner = CliRunner()
        result = runner.invoke(extract_pdf_text_cmd, ['--pdf-url', 'https://example.com/paper.pdf'])
...
class TestUtilityCommands:
    ...
    def test_clean_text_cmd_success(self, mock_clean_text):
        ...
        runner = CliRunner()
        result = runner.invoke(clean_text_cmd, [
            '--text', '  messy   text  ',
            '--email', 'test@example.com'
        ])
    ...
    def test_extract_doi_from_url_cmd_success(self, mock_extract_doi_from_url):
        ...
        runner = CliRunner()
        result = runner.invoke(extract_doi_from_url_cmd, ['--doi-url', 'https://doi.org/10.1038/nature12373'])
        
        assert result.exit_code == 0
        mock_extract_doi_from_url.assert_called_once_with('https://doi.org/10.1038/nature12373')
        output = json.loads(result.output.strip())
        assert output == "10.1038/nature12373"
...
class TestSearchCommands:
    ...
    def test_search_papers_by_keyword_cmd_basic(self, mock_search_papers_by_keyword):
        ...
        runner = CliRunner()
        result = runner.invoke(search_papers_by_keyword_cmd, ['--query', 'machine learning'])
        
        assert result.exit_code == 0
        mock_search_papers_by_keyword.assert_called_once_with(
            query='machine learning', max_results=20, sort='relevance', filter_params=None
        )
    
    @patch('artl_mcp.cli.search_papers_by_keyword')
    def test_search_papers_by_keyword_cmd_with_options(self, mock_search_papers_by_keyword):
        """Test keyword search with all options."""
        mock_search_papers_by_keyword.return_value = {"message": {"items": []}}
        
        runner = CliRunner()
        result = runner.invoke(search_papers_by_keyword_cmd, [
        ...
        mock_search_papers_by_keyword.assert_called_once_with(
            query='artificial intelligence', max_results=10, sort='published', filter_params=expected_filters
        )
    ...
    def test_search_recent_papers_cmd_basic(self, mock_search_recent_papers):
        ...
        mock_search_recent_papers.assert_called_once_with(
            query='deep learning', years_back=5, max_results=20, paper_type='journal-article'
        )
    ...
    def test_search_recent_papers_cmd_with_options(self, mock_search_recent_papers):
        ...
        runner = CliRunner()
        result = runner.invoke(search_recent_papers_cmd, [
        ...
        assert result.exit_code == 0
        mock_search_recent_papers.assert_called_once_with(
            query='neural networks', years_back=5, max_results=15, paper_type='journal-article'
        )
